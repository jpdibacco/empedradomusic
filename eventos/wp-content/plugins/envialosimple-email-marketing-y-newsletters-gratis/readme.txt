=== EnvialoSimple: Email Marketing y Newsletters ===
Contributors: donweb.com
Tags: email,email marketing,emailing,newsletter,envialosimple,plugin,editor visual,drag,drop,importar,contactos,posts,wysiwyg
Requires at least: 3.3
Tested up to: 4.4
Stable tag: trunk
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

El plugin de EnvialoSimple te permitirá crear y enviar Newsletters de calidad profesional, en minutos y directamente desde tu Wordpress.

== Description ==

**Envía Campañas GRATIS** Te regalamos 1.000 envíos mensuales para que envíes tus Campañas y hagas crecer tus visitas.

✔ Edición "Arrastrar y Soltar"

✔ Más de 200 Templates profesionales

✔ Campañas personalizables

✔ Reportes completos

✔ Importación de Contactos

✔ Robusta plataforma de envío

✔ Form de Suscripción con widget

✔ Programación de Envíos

✔ Instalacion rápida y amigable



= Características Generales =

* **Simple e Intuitivo** Gracias a su interfaz minimalista e intuitiva, podrás crear y enviar Newsletters de forma práctica y eficiente.

* **Completos y poderosos reportes** Accede a estadísticas de aperturas, clicks, rebotes, suscripciones, ubicación geográfica, cliente de correo, y mucho más.

* **Rápido y Eficaz** Una infraestructura especialmente desarrollada para que tus Campañas lleguen a la bandeja de entrada de tus suscriptores, en tiempo y forma.

* **Editor de Contenido WYSIWYG** Te permite incluir tus posts de Wordpress arrastrando y soltando, y sin conocimientos de HTML.

* **Más de 200 plantillas gratuitas** Una extensa galería de diseños de calidad profesional, categorizados y listos para usar con tus campañas.

* **Gestión de listas y contactos** Importa tus contactos de Wordpress y arma listas de Email para enviarles tus campañas.

* **Vista Previa** Visualiza como tus destinatarios verán el contenido de la campaña, antes de enviarlo.

* **Soporte tecnico** Soporte técnico las 24 hs y los 365 días del año.


Si aún no tienes cuenta de EnvialoSimple, podrás crear una en forma gratuita desde el plugin y obtendrás 1.000 envíos mensuales de regalo.


Más información del producto en: [EnvialoSimple](http://envialosimple.com/envialosimple-wordpress-plugin)

== Installation ==

1. Agregar el Plugin utilizando la Administración de Wordpress, o bien subiéndolo por FTP.
2. Activar el Plugin, desde la sección Plugins de Wordpress.
3. Ir al Menú EnvialoSimple -> Configuración .
4. Ingrese los datos de su Cuenta, o bien cree una Cuenta Nueva Gratis.
5. Ya puede comenzar a crear y enviar sus Newsletters.

== Screenshots ==

1. Pantalla de Configuración Inicial.
2. Vista del Editor de Campañas con una Plantilla.
3. Seleccionando un bloque para su Edición.
4. Editando Texto.
5. Arrastrando y Soltando un Nuevo Bloque de Contenido Wordpress.
6. Pantalla de Selección de Posts para Incorporar a la Campaña.
7. Vista del Post Agregado en el Contenido.
8. Pantalla de Importar Contactos. 
9. Pantalla de Formularios de Suscripción. 

