...
$campoBusqueda = isset($_REQUEST['filter']) ? $_REQUEST['filter'] : __('Buscar Campañas...', 'envialo-simple');
...
<?php if (isset($_GET["configurado"])): ?>
    <div id='msj-respuesta' class='mensaje msjExito' style='width: 55%; display:inline-block'>
        <?php _e('El Plugin ha sido Configurado Correctamente! Ya podés comenzar a Enviar tus Campañas.', 'envialo-simple'); ?>
    </div>
<?php endif; ?>
...
<div class="wrap">
    ...
    <h2 style="float:left">
        <?php _e('Campañas', 'envialo-simple'); ?>
        <a href="<?php echo get_admin_url() . "admin.php?page=envialo-simple-nuevo" ?>" class="add-new-h2 abrir-modal-campana"><?php _e('Crear Nueva Campaña', 'envialo-simple'); ?></a>
    </h2>
    ...
    <div class="busqueda-campanas">
        ...
        <div id="filter-news">
            <span><?php _e('Mostrar únicamente Campañas creadas desde el blog?', 'envialo-simple') ?></span>
    ...
    <div class="tool-box" id="contenedor-newsletters">
        <?php if (isset($_GET["camp-enviada"])): ?>
            <div id='msj-respuesta' class='mensaje msjExito' style='width: 55%; display:inline-block'>
                <?php _e('Campaña Enviada Correctamente !!', 'envialo-simple'); ?>
            </div>
        <?php endif; ?>
